...
func (b *boomware) request(method, apiMethod string, request interface{}, response interface{}) *Error {

	url := b.endpoint + apiMethod

	jsonBytes, err := json.Marshal(request)
	if err != nil {
		return NewError(MarshalRequestErrorCode, fmt.Sprintf("failed to marshal request:%s error:%s", apiMethod, err.Error()))
	}

	req, err := http.NewRequest(method, url, bytes.NewBuffer(jsonBytes))
	if err != nil {
		return NewError(CreateRequestErrorCode, fmt.Sprintf("failed to create http request:%s error:%s", apiMethod, err.Error()))
	}
	...
	if err != nil {
		return NewError(DoRequestErrorCode, fmt.Sprintf("http request:%s error %s", apiMethod, err.Error()))
	}
	defer func() {
		_ = resp.Body.Close()
	}()
	...
	if err != nil {
		return NewError(ReadBodyErrorCode, fmt.Sprintf("failed to read body request:%s error %s", apiMethod, err.Error()))
	}
	...
	if b.isCodeSucceeded(resp.StatusCode) == false {
		...
		if err != nil {
			return NewError(UnmarshalErrorCode, fmt.Sprintf("failed to unmarshal request:%s error:%s", apiMethod, err.Error()))
		}
		return apiErr
	}
